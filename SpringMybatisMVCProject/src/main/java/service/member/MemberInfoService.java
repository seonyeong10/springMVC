...
public class MemberInfoService {
	...
	public void execute(HttpSession session, Model model) {
		/**
		 * session 정보는 authInfo에 저장되어있다. 내 정보를 가져오려면 어떤 쿼리문 필요? -> 회원 하나만 select한 쿼리문 필요
		 * -> 이미 존재하는가? -> selectByMember Mapper.xml에 가서 조건절 확인 -> 조건에 맞는 데이터 만들기 -> 쿼리문
		 * 사용
		 */
	...
	public String pwUpdate(String userPw, HttpSession session, Model model) {
		MemberDTO memberDTO = new MemberDTO();
		AuthInfo authInfo = (AuthInfo) session.getAttribute("authInfo");
		memberDTO.setUserId(authInfo.getUserId());
		memberDTO = memberRepository.selectByMember(memberDTO);
		if (bcryptPasswordEncoder.matches(userPw, memberDTO.getUserPw())) {
			model.addAttribute("memberPwCommand", new MemberPwCommand());	// 에러 객체 반환할 때 에러 방지
